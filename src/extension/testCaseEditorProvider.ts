        ...
        case 'ConfirmRequest': {
          ...
          switch (action.kind) {
            ...
            case 'ResetContextVar':
              ...
              successAction = 'Reset';
              break;
            case 'ReplaceOriginalWithLosses': {
              const { dropped_assertions, unset_fields, dropped_tests } =
                action.value;
              const lines = [
                ...dropped_tests.map((t) =>
                  vscode.l10n.t(
                    'the test {0}: it could not be rebuilt against the scope',
                    t
                  )
                ),
                ...dropped_assertions.map((f) =>
                  vscode.l10n.t(
                    'the assertion on {0}: the rebuild has no expected value for it',
                    f
                  )
                ),
                ...unset_fields.map((f) =>
                  vscode.l10n.t(
                    'the value of {0}: still unset, written as impossible',
                    f
                  )
                ),
              ];
              prompt = vscode.l10n.t(
                'Replacing the original will lose what is listed below. Replace anyway?'
              );
