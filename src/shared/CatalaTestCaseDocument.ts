  ...
  implements vscode.CustomDocument
{
  ...
  static async create(
    ...
    const document = new CatalaTestCaseDocument(uri, fileData);
    // A repair the test no longer needs; the next drift would pick it up.
    if (document._parseResults.kind === 'Results') {
      await document.deleteWorkingCopy({ useTrash: true });
    }
    return document;
  }
  ...
  private cancelPendingEdits(): void {
  ...
  async revert(_cancellation: vscode.CancellationToken): Promise<void> {
    ...
    this._sourceText = new TextDecoder('utf-8').decode(diskContent);
   ...
   */
  dispose(): void {
    // A timer must not land an edit on a closed document.
  ...
  _commitRebuilt(tests: TestList): void {
    ...
    this._rebuilt = tests;
    const step = (rebuilt: TestList | undefined): void => {
      // Inert once the original was replaced; a pending edit is newer.
      if (this._parseResults.kind !== 'BrokenTest') return;
      this.cancelPendingEdits();
      this._rebuilt = rebuilt;
      this._onDidChangeDocument.fire({ document: this });
    };
    this._onDidChange.fire({
      document: this,
      label: 'rebuild',
      undo: (): void => step(previous),
      redo: (): void => step(tests),
    });
  }
  ...
  /** The text the parse results came from: the buffer a rebuild must read. */
  ...
  public retarget(results: ParseResults): void {
    ...
    const stamped = stampParseResultsUids(results);
    const next = { results: stamped, rebuilt: rebuiltFrom(stamped) };
    const apply = (s: typeof next): void => {
      this._parseResults = s.results;
      this._rebuilt = s.rebuilt;
      this._onDidChangeDocument.fire({ document: this });
    };
    const step = (s: typeof next): void => {
      if (this._parseResults.kind !== 'BrokenTest') return;
      this.cancelPendingEdits();
      apply(s);
    };
  ...
  private async deleteWorkingCopy(
    options: { useTrash?: boolean } = {}
  ): Promise<void> {
    try {
      await vscode.workspace.fs.delete(this.workingCopyUri, options);
    } catch (err) {
      // FileNotFound is normal; a stale .repair outliving a Replace is not.
      if (err instanceof vscode.FileSystemError && err.code === 'FileNotFound')
        return;
      // No trash on some remotes: a plain delete then.
      if (options.useTrash) return this.deleteWorkingCopy();
      logger.log(
  ...
  private constructor(uri: vscode.Uri, initialContent: Uint8Array) {
    ...
    this._sourceText = new TextDecoder('utf-8').decode(initialContent);
