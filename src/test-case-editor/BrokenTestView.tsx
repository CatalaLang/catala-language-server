...
/** Constructors the value's enum had that the live one lacks, when the
 *  change is within one enum. */
function goneCtors([from, to]: [Typ, Typ]): string[] {
  if (from.kind !== 'TEnum' || to.kind !== 'TEnum') return [];
  if (from.value.enum_name !== to.value.enum_name) return [];
  return [...from.value.constructors.keys()].filter(
    (c) => !to.value.constructors.has(c)
  );
}
...
function CarryMark({
  ...
  let base: string;
  const gone = outcome.kind === 'TypeChanged' ? goneCtors(outcome.value) : [];
  if (outcome.kind === 'TypeChanged' && gone.length > 0) {
    // Same enum on both sides: the type names would say nothing.
    base = intl.formatMessage(
      { id: 'broken.markCtorGone' },
      {
        enum: getTypeDisplayName(outcome.value[1], intl),
        ctors: gone.join(', '),
      }
    );
  } else {
    const change =
      outcome.kind === 'TypeChanged'
        ...
        : '';
    base = intl.formatMessage({ id }, { change });
  }
  const title =
  ...
  switch (note.kind) {
    ...
    case 'WorkingCopyRecovered':
      return (
        <>
          <FormattedMessage
            id="broken.noteWorkingCopyRecovered"
            values={{ name: <code>{note.value.name}</code> }}
          />
          {note.value.lost.length > 0 && (
            <>
              {' '}
              <FormattedMessage
                id="broken.noteWorkingCopyLost"
                values={{ names: <code>{note.value.lost.join(', ')}</code> }}
              />
            </>
          )}
        </>
          ...
          <FormattedMessage
            id={
              note.kind === 'ModuleWontCompile'
                ...
                : note.kind === 'WorkingCopyUnreadable'
                  ? 'broken.noteWorkingCopyUnreadable'
                  : 'broken.noteOther'
            }
            values={{ name: <code>{note.value.name}</code> }}
          />
