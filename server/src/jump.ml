...
let pp_module_jump fmt { interface; alias; _ } =
  let open Format in
  fprintf fmt "%s %a"
...
let make_mjump
    ...
    ?alias
    k =
  (* TODO: print positions to make sure. *)
  let mjump : mjump =
    let hash = hash_info (module ModuleName) mname in
...
let pp_var ppf =
  ...
  | Module_use mjump -> fprintf ppf "moduse: %a" pp_module_jump mjump
  | Module_decl mjump -> fprintf ppf "moddecl: %a" pp_module_jump mjump
  | Module_def mjump -> fprintf ppf "moddef: %a" pp_module_jump mjump
...
(* let path_with_pos full_pos = assert false *)
...
let populate
  ...
  let variables, mod_lookup =
    populate_modules modules_itf prog surface PMap.empty
  in
