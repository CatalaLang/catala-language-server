...
describe('TableArrayEditor - Main table should render all items including Impossible/Unset/Invalid', () => {
  it('should render all 8 parent items in main table (5 computed + 3 impossible)', () => {
    ...
    // Look for all row numbers in the table
    // Each row has a row number cell with text like "#1", "#2", etc.
    const rowNumbers = screen.queryAllByText(/#\d+/i);

    console.log('Row numbers found:', rowNumbers.length);
    ...
    expect(rowNumbers.length).toBeGreaterThanOrEqual(8);
  });
});
