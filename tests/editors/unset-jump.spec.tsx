...
import { arrayVal, enumVal, intVal, rv, structVal } from './test-helpers';
...
describe('firstUnsetPath', () => {
  ...
  it('names the payload on the way into an enum', () => {
    const wrapDecl: EnumDeclaration = {
      enum_name: 'Wrap',
      constructors: new Map([['Some', { value: pointTyp }]]),
      ctor_attrs: new Map(),
    };
    const test: Test = {
      testing_scope: 'T',
      tested_scope: {
        ...
        module_name: 'M',
        inputs: new Map([
          ['w', { typ: { kind: 'TEnum', value: wrapDecl }, is_context: false }],
        ]),
        outputs: new Map(),
        module_deps: [],
      },
    ...
    expect(firstUnsetPath(test)).toEqual([
      { kind: 'StructField', value: 'w' },
      { kind: 'EnumPayload', value: 'Some' },
      { kind: 'StructField', value: 'x' },
    ]);
  });
});
