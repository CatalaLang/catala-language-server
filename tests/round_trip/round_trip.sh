...
# a rule, a conditional definition: not literals, so the partial read refuses
out=$(cd "$ref_scratch" && catala testcase partial-read test_flags.catala_en 2>&1)
echo "$out" | grep -q 'rule' || { echo "FAIL: a rule was not refused by the partial read: $out"; exit 1; }
# a test whose tested scope names no module
out=$(cd "$ref_scratch" && catala testcase read test_local.catala_en 2>&1)
echo "$out" | grep -q 'names no module' || { echo "FAIL: an unqualified tested scope: $out"; exit 1; }
echo "$out" | grep -qE "$crash" && { echo "FAIL: an unqualified tested scope crashed"; exit 1; }
# a function-typed input
out=$(cd "$ref_scratch" && catala testcase list-scopes fn.catala_en 2>&1)
echo "$out" | grep -q 'function type' || { echo "FAIL: a function input was not refused: $out"; exit 1; }
echo "$out" | grep -qE "$crash" && { echo "FAIL: a function input crashed"; exit 1; }
# a list mixing element types
out=$(cd "$ref_scratch" && catala testcase partial-read test_lists.catala_en 2>&1)
echo "$out" | grep -q 'mixing' || { echo "FAIL: a mixed list was not refused: $out"; exit 1; }
