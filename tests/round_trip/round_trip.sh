...
# ── a diff inside an optional names the payload ─────────────────────────────
# The wire addresses the payload explicitly (EnumPayload), so exactly one
# editor node owns each diff.
clerk test test_opt_out.catala_en >/dev/null 2>&1 || true  # build the plugin; the test is meant to fail
out=$(catala testcase run --scope Give_test test_opt_out.catala_en)
echo "$out" | grep -q '"assert_failures":true' \
    || { echo "FAIL: the wrong expectation did not fail"; exit 1; }
echo "$out" | grep -q '\["EnumPayload","Present"\]' \
    || { echo "FAIL: the diff path does not name the payload"; exit 1; }

# ── a bare constructor of a user enum survives input serialization ─────────
# The input filter drops an absent option, not every payload-less constructor.
catala testcase read test_tint.catala_en \
    | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));
               process.stdout.write(JSON.stringify(d[0].test_inputs))' \
    > tint_inputs.json
...
# must hear about clerk, not fall through to the interpreter's own hint. The
# editor's clerk comes through CATALA_CLERK_PATH; a wrong one must be reported.
prep_scratch=$(mktemp -d)
cp clerk.toml tint.catala_en test_tint.catala_en "$prep_scratch"/
out=$(cd "$prep_scratch" \
    && env CATALA_CLERK_PATH=/nonexistent/clerk catala testcase run \
        --scope Paint_tint test_tint.catala_en 2>&1 >/dev/null)
rm -rf "$prep_scratch"
echo "$out" | grep -q 'Could not prepare' \
    || { echo "FAIL: a failed clerk preparation was not reported"; exit 1; }

# ── a literal the reader cannot type is a located refusal, not a crash ─────
bad_scratch=$(mktemp -d)
cp clerk.toml details.catala_en "$bad_scratch"/
sed 's/|2026-01-01|/|2026-02-30|/' test_details.catala_en > "$bad_scratch/test_details.catala_en"
out=$(cd "$bad_scratch" && clerk start >/dev/null 2>&1; catala testcase rebuild test_details.catala_en 2>&1 >/dev/null)
rm -rf "$bad_scratch"
echo "$out" | grep -q 'test_details.catala_en:15' \
    || { echo "FAIL: an unreadable literal was not located: $out"; exit 1; }

# ── the rebuild reads the buffer it is given, not the file on disk ─────────
sed 's/calc\.base equals \$1000\.00/calc.base equals $1234.00/' test_optionals.catala_en \
    | catala testcase rebuild --buffer-path test_optionals.catala_en - 2>/dev/null \
    | grep -q '123400' \
    || { echo "FAIL: the rebuild did not read the buffer on stdin"; exit 1; }

# ── refusals and drifts that must be answers, not crashes ──────────────────
ref_scratch=$(mktemp -d)
cp refusals/* "$ref_scratch"/
( cd "$ref_scratch" && clerk start >/dev/null 2>&1 )
crash='Unexpected error|Assert_failure|Not_found|Failure'
refuses() { # command file expected-text
    out=$(cd "$ref_scratch" && catala testcase "$1" "$2" 2>&1)
    echo "$out" | grep -q "$3" || { echo "FAIL: $1 $2 should say '$3': $out"; exit 1; }
    echo "$out" | grep -qE "$crash" && { echo "FAIL: $1 $2 crashed: $out"; exit 1; }
}
refuses partial-read test_flags.catala_en 'rule'
refuses read test_local.catala_en 'names no module'
refuses list-scopes fn.catala_en 'function type'
refuses partial-read test_lists.catala_en 'mixing'
# the writer on values that do not fit: a refusal, and a dropped override
json=$(cd "$ref_scratch" && catala testcase read test_moods.catala_en)
bad=$(echo "$json" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));d[0].test_inputs.shade.value.value.value[1][1][0]="Purple";process.stdout.write(JSON.stringify(d))')
out=$(cd "$ref_scratch" && echo "$bad" | catala testcase write -l en 2>&1)
echo "$out" | grep -q 'do not fit' || { echo "FAIL: write accepted an unknown constructor: $out"; exit 1; }
echo "$out" | grep -qE "$crash" && { echo "FAIL: write crashed on an unknown constructor"; exit 1; }
nov=$(echo "$json" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));d[0].test_outputs.is_red.value.value.value="NotOverridden";process.stdout.write(JSON.stringify(d))')
out=$(cd "$ref_scratch" && echo "$nov" | catala testcase write -l en 2>&1) || { echo "FAIL: write failed on a not-overridden output: $out"; exit 1; }
echo "$out" | grep -q 'assertion' && { echo "FAIL: a not-overridden output was written as an assertion"; exit 1; }
# drift: the same constructor in another enum does not carry; a record that
# became optional and changed inside carries piece by piece
sed -i 's/input shade content Colour/input shade content Mood/; s/input box content Box/input box content optional of Box/; s/data gone content integer/data added content integer/; s/-- Colour.Red : true/-- Mood.Red : true/; s/-- Colour.Green : false/-- Mood.Blue : false/' "$ref_scratch/moods.catala_en"
( cd "$ref_scratch" && clerk start >/dev/null 2>&1 )
( cd "$ref_scratch" && catala testcase rebuild test_moods.catala_en 2>/dev/null ) \
  | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));const t=(Array.isArray(d)?d[1]:d).tests[0];
      const name=(c)=>c.path.map((p)=>p[1]).join(".");const kind=(c)=>Array.isArray(c.outcome)?c.outcome[0]:c.outcome;
      const by={};for(const c of t.outcomes){by[name(c)]=kind(c);}
      if(by["shade"]!=="TypeChanged"){console.error("FAIL: a same-named constructor of another enum carried: "+JSON.stringify(by));process.exit(1);}
      if(by["box"]!=="Partial"){console.error("FAIL: a record that became optional and changed did not carry piece by piece: "+JSON.stringify(by));process.exit(1);}
      if(by["box.Present.gone"]!=="Dropped"||by["box.Present.added"]!=="WasUnset"){console.error("FAIL: marks inside the new option: "+JSON.stringify(by));process.exit(1);}' \
  || exit 1
# an unreadable working copy is a note, not silently ignored
printf '```catala\nscope Broken:\n  definition\n```\n' > "$ref_scratch/test_moods.catala_en.repair"
( cd "$ref_scratch" && catala testcase rebuild test_moods.catala_en 2>/dev/null ) | grep -q 'WorkingCopyUnreadable' \
  || { echo "FAIL: an unreadable working copy was ignored without a note"; exit 1; }
rm -rf "$ref_scratch"
...
# ── a working copy the scope drifted under again ────────────────────────────
# Drift one renames base; the tester fills amount and edits bonus in the
# working copy. Drift two renames it back. The working copy is recovered too:
# its bonus wins, base comes from the original, amount is named as lost.
mkdir -p "$notes_scratch/twice"
cp clerk.toml test_optionals.catala_en "$notes_scratch/twice"/
sed 's/\bbase\b/amount/g' optionals.catala_en > "$notes_scratch/twice/optionals.catala_en"
(cd "$notes_scratch/twice" && clerk start >/dev/null 2>&1 \
    && catala testcase rebuild test_optionals.catala_en 2>/dev/null) \
  | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));
      const tests=d.tests.map((t)=>t.rebuilt);
      for(const t of tests){
        t.test_inputs.amount.value={value:{value:["Money",123400],attrs:[]}};
        t.test_inputs.bonus.value={value:{value:["Enum",[{enum_name:"Optional",constructors:{Absent:"None",Present:["Some","TMoney"]}},["Present",["Some",{value:["Money",5000],attrs:[]}]]]],attrs:[]}};
      }
      process.stdout.write(JSON.stringify(tests))' \
  | (cd "$notes_scratch/twice" && catala testcase write -l en > test_optionals.catala_en.repair) \
  || { echo "FAIL: could not write the first working copy"; exit 1; }
cp optionals.catala_en "$notes_scratch/twice"/
(cd "$notes_scratch/twice" && clerk start >/dev/null 2>&1 \
    && catala testcase rebuild test_optionals.catala_en 2>/dev/null) \
  | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));
      const fail=(m)=>{console.error("FAIL: "+m+" "+JSON.stringify(d.notes));process.exit(1);};
      const n=d.notes.find((n)=>n[0]==="WorkingCopyRecovered");
      if(!n) fail("the drifted working copy was not recovered");
      if(!n[1].lost.includes("amount")) fail("the lost edit was not named");
      const t=d.tests.find((t)=>t.authored.testing_scope==="Grant_absent").rebuilt;
      const v=(f)=>JSON.stringify(t.test_inputs[f].value.value.value);
      if(!v("base").includes("100000")) fail("base should come from the original: "+v("base"));
      if(!v("bonus").includes("5000")) fail("the working copy edit to bonus was lost: "+v("bonus"));' \
  || exit 1

# ── a field renamed ─────────────────────────────────────────────────────────
# What each field becomes is the carry table's business. The rebuild adds: no
# note for an ordinary rename, and silence on an output a test never asserted.
...
node -e '
  ...
  const total = d.tests.flatMap((t) => t.outcomes).filter((c) => c.path.map((p) => p[1]).join(".") === "total");
  if (total.length !== 2) { console.error("FAIL: an unasserted output was reported: " + JSON.stringify(total)); process.exit(1); }
...
# Like clerk, the plugin expects to run from within the project: clerk's scan
# reads the include dirs relative to the cwd. A run from above the root is not
# exercised until the plugin rebases itself the way clerk does.
...
# requires a payload. Whatever a rebuild carries, its working copy reads back.
...
for dir in lostfield payload; do
    node -e 'process.stdout.write(JSON.stringify(JSON.parse(require("fs").readFileSync(process.argv[1], "utf8")).tests.map((t) => t.rebuilt)))' \
        "$notes_scratch/$dir.json" \
