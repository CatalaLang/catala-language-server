...
let absent inner =
  O.{ value = Enum (Testcase.Model.mk_optional_enum_decl inner, (Testcase.Model.option_absent, None)); attrs = [] }

let present inner v =
  O.
    {
      value = Enum (Testcase.Model.mk_optional_enum_decl inner, (Testcase.Model.option_present, Some v));
      attrs = [];
    }
...
let enum_named enum_name ctors = O.{ enum_name; constructors = ctors; ctor_attrs = [] }
...
let rows =
  [
    ...
    };
    {
      ...
      old_typ = TEnum (enum_named Testcase.Model.unknown_enum_name ["Red", None]);
      new_typ = TEnum (enum_named "M.Mood" ["Red", None]);
      value = red (enum_named Testcase.Model.unknown_enum_name ["Red", None]);
      outcome = Fits; carried = true;
    };
    {
      ...
      value = struct_of O.{ struct_name = "M.Other"; fields = ["x", TInt] } ["x", int 1];
      outcome =
        TypeChanged
...
let show_outcome : O.carry_outcome -> string = function
  ...
  | TypeChanged (a, b) ->
    Printf.sprintf "TypeChanged (%s -> %s)" (Testcase.Model.typ_name a) (Testcase.Model.typ_name b)
  | Dropped -> "Dropped"
  | Partial -> "Partial"

let check_row r =
  let carried, outcome, _ =
    Testcase.Recovery.carry_value ~old_typ:r.old_typ ~new_typ:r.new_typ r.value
  in
...
let check_adopts_live_declarations () =
  ...
  match
    Testcase.Recovery.carry_value ~old_typ:(TEnum (colour ["Red", None]))
...
let check_keeps_attributes () =
  ...
  match Testcase.Recovery.carry_value ~old_typ:TMoney ~new_typ:(TOption TMoney) v with
...
let check_carries_inside_records () =
  ...
  in
  match Testcase.Recovery.carry_value ~old_typ ~new_typ:(O.TStruct new_decl) v with
...
let check_carries_inside_payloads () =
  ...
  let check what ~old_typ ~new_typ ~ctor value =
    match Testcase.Recovery.carry_value ~old_typ ~new_typ value with
    | Some { value = Enum (_, (c, Some { value = Struct (decl, _); _ })); _ }, Partial, nested ->
      ...
      let expect p o =
        if not (List.mem (p, o) nested) then
          failwith (Printf.sprintf "%s: missing nested outcome %s" what (show_outcome o))
      in
      expect [`EnumPayload ctor; `StructField "second"] Dropped;
      expect [`EnumPayload ctor; `StructField "amount"] WasUnset;
      if List.length nested <> 2 then failwith (what ^ ": unexpected nested outcomes")
    | _, outcome, _ ->
      failwith
        (Printf.sprintf "%s: expected Partial, got %s" what (show_outcome outcome))
  in
  let old_colour = colour ["Red", Some (O.TStruct old_detail)] in
  let new_colour = colour ["Red", Some (O.TStruct new_detail)] in
  check "enum payload" ~old_typ:(O.TEnum old_colour) ~new_typ:(O.TEnum new_colour)
    ~ctor:"Red" (red_of old_colour v);
  check "option payload" ~old_typ:(O.TOption (O.TStruct old_detail))
    ~new_typ:(O.TOption (O.TStruct new_detail)) ~ctor:Testcase.Model.option_present
...
let check_carries_inside_lists () =
  ...
  in
  (match
     Testcase.Recovery.carry_value ~old_typ:(O.TArray old_elt) ~new_typ:(O.TArray new_elt)
       (arr [elt; elt])
   with
  ...
  | _, outcome, _ ->
    failwith
      (Printf.sprintf "a list of changed records: expected Partial, got %s"
         (show_outcome outcome)));
  match
    Testcase.Recovery.carry_value ~old_typ:(O.TArray O.TMoney) ~new_typ:(O.TArray O.TDate)
      (arr [money 1; money 2])
  with
  | None, TypeChanged _, [] -> ()
  | _, outcome, nested ->
...
let check_nested_shapes () =
  ...
  | Some { value = Array [| _ |]; _ }, Partial, nested ->
    expect_nested "list of options" nested
      [`ListIndex 0; `EnumPayload Testcase.Model.option_present; `StructField "c"]
      O.WasUnset;
  ...
  (match
     Testcase.Recovery.carry_value ~old_typ:(TTuple [TInt; TMoney]) ~new_typ:(TTuple [TInt; TDate])
       (arr [int 1; money 5])
   with
  ...
  (match
     Testcase.Recovery.carry_value ~old_typ:(TStruct old_s) ~new_typ:(TStruct new_s)
       (struct_of old_s ["a", int 1])
   with
  | Some { value = Struct (_, ["a", { value = Enum (_, (ctor, Some { value = Integer 1; _ })); _ }]); _ },
    Partial, nested when ctor = Testcase.Model.option_present ->
  ...
  (match
     Testcase.Recovery.carry_value ~old_typ:(TOption (TStruct old_s)) ~new_typ:(TOption (TStruct new_s))
  ...
  | Some { value = Enum (_, (_, Some { value = Struct (_, fields); _ })); _ }, Partial, nested ->
    ...
    if nested <> [[`EnumPayload Testcase.Model.option_present; `StructField "b"], O.Dropped] then
...
let check_hole_keeps_attributes () =
  ...
  match
    Testcase.Recovery.carry_value ~old_typ:(TArray (TEnum narrow)) ~new_typ:(TArray (TEnum live))
...
(* A record that became optional, or stopped being one, and changed inside:
   the payload carries piece by piece across the option boundary. *)
let check_carries_across_option () =
  ...
  (match
     Testcase.Recovery.carry_value ~old_typ:(TStruct old_s) ~new_typ:(TOption (TStruct new_s)) v
   with
  | Some { value = Enum (_, (ctor, Some { value = Struct (_, fields); _ })); _ }, Partial, nested
    when ctor = Testcase.Model.option_present ->
    if List.assoc_opt "x" fields <> Some (int 1) then failwith "wrapped: x lost";
    expect_nested "wrapped" nested [`EnumPayload Testcase.Model.option_present; `StructField "gone"] O.Dropped;
    expect_nested "wrapped" nested [`EnumPayload Testcase.Model.option_present; `StructField "added"] O.WasUnset
  | _, outcome, nested ->
    failwith (Printf.sprintf "wrapped: %s, nested = %s" (show_outcome outcome) (show_nested nested)));
  match
    Testcase.Recovery.carry_value ~old_typ:(TOption (TStruct old_s)) ~new_typ:(TStruct new_s)
      (present (TStruct old_s) v)
  with
  | Some { value = Struct (_, fields); _ }, Partial, nested ->
    if List.assoc_opt "x" fields <> Some (int 1) then failwith "unwrapped: x lost";
    expect_nested "unwrapped" nested [`StructField "gone"] O.Dropped;
    expect_nested "unwrapped" nested [`StructField "added"] O.WasUnset
  | _, outcome, nested ->
    failwith (Printf.sprintf "unwrapped: %s, nested = %s" (show_outcome outcome) (show_nested nested))
