...
let cmd_generate =
  Cmd.v
    ...
    Term.(
      const (fun a b c d e ->
          Project.guarded (fun () ->
              Commands.generate_cmd ?testing_scope:None a b c d e))
      $ Cli.Flags.ex_scope
...
let cmd_rebuild =
  Cmd.v
    ...
    Term.(
      const Recovery.rebuild_broken_test
      $ Cli.Flags.Global.options
      $ retarget
      $ buffer_path)
      ...
      info "partial-read"
        ~doc:
          "Read the tests of a file from its surface syntax alone, without \
           ...
           is what the file itself proves and no more. This is the reader that \
           still works when the scope under test has changed underneath the \
...
let cmd_write =
  Cmd.v
    ...
    Term.(
      const (fun a b -> Project.guarded (fun () -> Model.write_catala a b))
...
let cmd_list_scopes =
  Cmd.v
    ...
    Term.(
      const (fun a b -> Project.guarded (fun () -> Commands.list_scopes a b))
      $ Cli.Flags.include_dirs
      $ Cli.Flags.Global.options)
...
let cmd_serialize_inputs =
  Cmd.v
    ...
    Term.(const Commands.serialize_inputs $ Cli.Flags.scope_input)
...
let register () =
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Model.Uid s)
  ...
  (Driver.Plugin.register_attribute ~plugin:"testcase" ~path:["testui"]
     ...
     | _ -> false)
  @@ fun ~pos:_ value -> match value with _ -> Some Model.TestUi);
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Model.TestDescription s)
