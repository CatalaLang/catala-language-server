...
let stub_modules (lang : Global.backend_lang) (sd : O.scope_def) :
  ...
  let kw_struct, kw_enum, kw_data =
    ...
    | _ -> unsupported "unsupported language"
  in
  (* A loud, visible (literate) banner: a stub is recovery scaffolding, never a
     real module — make that impossible to miss for anyone who opens the file. *)
  let stub_banner =
    "**⚠ AUTOGENERATED MIGRATION STUB — DO NOT EDIT, COMMIT, OR RUN.**\n\
     Generated by `catala testcase stub`. It re-declares this module's\n\
     *signature only*, so the testcase migration toolchain can typecheck and\n\
     RECOVER VALUES from a test whose real module is missing or has drifted.\n\
     It has no scope bodies — it does **not** compute anything; outputs are left\n\
      ...
      List.iter
        (fun (name, t) ->
          p "  %s %s %s %s\n" strings.output_scope name strings.content
            (render current t))
        sd.outputs;
      (* Outputs are intentionally left UNDEFINED: the stub recovers values, it
         does not compute, so emitting fake output bodies would misrepresent it as
         something that computes — and they are not needed to typecheck a test
         against the signature. Only context inputs (which carry a reentrant
         default) get a body; if there are none, no impl block is emitted at all. *)
      let ctx_defs =
        List.filter_map
          (fun (name, (si : O.scope_input)) ->
            if si.is_context then Some (name, si.typ) else None)
          sd.inputs
      in
      if ctx_defs = [] then p "```\n"
      else begin
