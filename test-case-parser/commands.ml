...
let rec convert_to_json_input ({ value; _ } : O.runtime_value) : Yojson.Safe.t =
  ...
  let convert_runtime_raw = function
    ...
    | Enum (_decl, (c, Some x)) when c = option_present -> convert_to_json_input x
...
let run_process ~cwd exe args =
  ...
  let close_w () = if not !closed then (closed := true; Unix.close w) in
  Fun.protect
    ~finally:(fun () -> close_w (); close_in_noerr ic)
    (fun () ->
      Sys.chdir cwd;
      let pid =
        Fun.protect
      ...
      | Unix.WSIGNALED _ | Unix.WSTOPPED _ -> 1, output ^ "\n(killed by a signal)")
