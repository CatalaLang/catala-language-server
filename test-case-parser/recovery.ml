...
let read_partial_test_one (scope_decl : Surface.Ast.scope_decl)
  ...
  let convert_definition : definition -> (string * O.test_io, string) Result.t =
    ...
    | { definition_state = Some _; _ } -> Error "a state definition is not a literal"
    | { definition_label = Some _; _ } -> Error "a labelled definition is not a literal"
...
let carry_rule ~(old_typ : O.typ) ~(new_typ : O.typ) (v : O.runtime_value)
  ...
  | _, O.TOption _ when not (match new_typ with O.TOption _ -> true | _ -> false)
    -> (
    match v.O.value with
    | O.Enum (_, (_, Some payload)) when value_fits new_typ payload = Ok () ->
      Some payload, O.Unwrap
...
and carry_inside ~(old_typ : O.typ) ~(new_typ : O.typ) (v : O.runtime_value) :
  ...
  let carry_at seg ~old_typ ~new_typ e =
    ...
    c
  in
  (* A payload is a node of its own, named by its constructor as diff paths
     name it; only the enum itself is marked when the payload does not carry. *)
  ...
  let result =
    ...
    | _, O.TOption ni, _ when (match old_typ with O.TOption _ -> false | _ -> true)
      ->
      Option.map
    ...
    | O.TOption oi, _, O.Enum (_, (_, Some p)) ->
      let c, _, sub = carry_value ~old_typ:oi ~new_typ p in
      nested := List.rev_append sub !nested;
      Option.map (fun (c : O.runtime_value) -> c.O.value) c
    | _ -> None
  in
  ...
  | Ok (recovered, errors) -> (
    ...
    | first :: _ -> (
      ...
      let saved =
        ...
        | Ok s -> s
        | Error e ->
          note (O.WorkingCopyUnreadable { O.name = workspace_file; error = e });
          None
      in
