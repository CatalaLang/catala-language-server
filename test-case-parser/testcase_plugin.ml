...
let cmd_generate =
  Cmd.v
    ...
    Term.(
      const (fun a b c d e ->
          Testcase.Project.guarded (fun () ->
              Testcase.Commands.generate_cmd ?testing_scope:None a b c d e))
      $ Cli.Flags.ex_scope
...
let cmd_read =
  Cmd.v
    ...
    Term.(
      const (fun a b c ->
          Testcase.Project.guarded (fun () -> Testcase.Commands.read_test a b c))
      $ Cli.Flags.include_dirs
      $ Cli.Flags.Global.options
      $ buffer_path)
...
let cmd_rebuild =
  Cmd.v
    ...
    Term.(
      const Testcase.Recovery.rebuild_broken_test
      $ Cli.Flags.Global.options
      $ retarget
      $ buffer_path)
...
let cmd_partial_read =
  Cmd.v
    ...
    Term.(const Testcase.Recovery.read_partial_test $ Cli.Flags.Global.options)
...
let cmd_run =
  Cmd.v
    ...
    Term.(
      const Testcase.Commands.run_test_cmd
      $ Cli.Flags.include_dirs
      $ Cli.Flags.Global.options
      $ Cli.Flags.ex_scope
...
let cmd_write =
  Cmd.v
    ...
    Term.(
      const (fun a b ->
          Testcase.Project.guarded (fun () -> Testcase.Model.write_catala a b))
...
let cmd_list_scopes =
  Cmd.v
    ...
    Term.(
      const (fun a b ->
          Testcase.Project.guarded (fun () -> Testcase.Commands.list_scopes a b))
      $ Cli.Flags.include_dirs
      $ Cli.Flags.Global.options)
...
let cmd_serialize_inputs =
  Cmd.v
    ...
    Term.(const Testcase.Commands.serialize_inputs $ Cli.Flags.scope_input)
...
let register () =
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Testcase.Model.Uid s)
  ...
  (Driver.Plugin.register_attribute ~plugin:"testcase" ~path:["testui"]
     ...
     | _ -> false)
  @@ fun ~pos:_ value ->
  match value with _ -> Some Testcase.Model.TestUi);
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Testcase.Model.TestDescription s)
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Testcase.Model.TestTitle s)
  ...
  @@ fun ~pos:_ value ->
  match value with
  | Shared_ast.String (s, _pos) -> Some (Testcase.Model.ArrayItemLabel s)
